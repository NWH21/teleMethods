...
def isBookmarkedMethod(call):
  return not isPackage(call) and not isClass(call) and not isBookmark(call) and "bookmark " in call.data
  
def isMethod(call):
  return not isPackage(call) and not isClass(call) and not isBookmark(call) and "bookmark " not in call.data
...
def keyboardForSearchingBookmarks(id):
  ...
  for methods in bookmarked_methods:
    button = InlineKeyboardButton(text = methods['method_name'],callback_data = "bookmark " + methods['method_name'])
    keyboard_inline.add(button)
  return keyboard_inline       
...
def bookmarks(message):
  ...
  if users.find_one({'chat_id' : message.chat.id}) != None:
      bot.send_message(message.chat.id,"Here are your bookmarks",reply_markup = keyboardForLookingUpBookmarks(message.chat.id)) 
  else:
    bot.send_message(message.chat.id,"You have no bookmarks")
      
@bot.callback_query_handler(func=isBookmarkedMethod)
def displayBookmarkedMethod(call):
  users = db["Bookmark"]
  id = call.message.chat.id
  bookmarked_methods = users.find_one({'chat_id' : id})["Bookmarked_methods"]
  for i in bookmarked_methods:
    if i['method_name'] == call.data.replace("bookmark ", ""):
      name = i["method_name"]
